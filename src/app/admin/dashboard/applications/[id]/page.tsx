          ...
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
            ...
            <div className="flex flex-col items-start md:items-end gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Submitted: {application.submissionDate ? format(new Date(application.submissionDate), 'dd MMM yyyy, p') : '-'}
              </p>
            </div>
          </div>
        </CardHeader>
                ...
                <>
                  ...
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                    {application.familyMembers.map((member, index) => (
                      <Card key={(member as any).id || index} className="bg-muted/30 p-3 text-sm">
                        <p><strong>{member.name}</strong> ({member.relation})</p>
                        <p>DOB: {format(new Date(member.dob), 'PPP')}</p>
                        {(member as any).dependent && <Badge variant="outline" className="mt-1 text-xs">Dependent</Badge>}
                      </Card>
                    ))}
                  </div>
                </>
